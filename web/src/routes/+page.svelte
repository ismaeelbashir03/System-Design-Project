...
<dialog class="modal" class:modal-open={showSummonModal}>
	<div class="modal-box flex flex-col gap-4">
		...
		<button on:click={async () => await summon('general')} class="btn bg-red-500 text-white"
			>General Waste</button
		>
		<button on:click={async () => await summon('recycling')} class="btn bg-green-500 text-white"
			>Recycling</button
		>
		<button on:click={async () => await summon('both')} class="btn bg-blue-500 text-white"
			>Both</button
		>
		<div class="modal-action">
